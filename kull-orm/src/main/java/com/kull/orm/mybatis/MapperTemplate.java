...
public  class MapperTemplate  {
  ...
  final	private String getJavaColsContext(){
		...
		for(Iterator<Class> it=this.mapJavaCols.keySet().iterator();it.hasNext();){
			...
			for(ColumnTemplate col :cols){
				...
				if(++index>3){
					index=0;
					sbrCols.append(StringHelper.ln(1)).append(StringHelper.tab(4));
				}
			}
  ...
  final private String getQueryColsContext(){
	...
	public String getCondition(ColumnTemplate ct){
		...
		//	.append(this.minDatePattern).append(StringHelper.ln(1))
		//	.append(this.maxDatePattern).append(StringHelper.ln(1))
		//	;
        }
    ...
    public String getMapperContext(){
       ...
       // .append(" public  class SimpleQueryModel extends {8} '{' \n {13} \n '}' ").append(StringHelper.ln(1))
        ...
        //.append(StringHelper.tab(3)).append(TAG_WHEN_END).append(StringHelper.ln(1))
        //.append(StringHelper.tab(3)).append(TAG_OTHERWISE_START).append(StringHelper.ln(1))
      ...
      //  .append(TAG_SELECT_END).append(StringHelper.ln(2))
       ...
       // .append(TAG_DELETE_END).append(StringHelper.ln(4))      
        ...
        .append(StringHelper.tab(1)).append("select m.* <include refid=\"joinColums\"/> from <include refid=\"table\" /> m <include refid=\"join\"/> where m.<include refid=\"pk\" /> not in").append(StringHelper.ln(1))
        .append(StringHelper.tab(1)).append("<foreach item=\"pk\" collection=\"list\" open=\"(\" separator=\",\" close=\")\"> #'{'pk'}' </foreach>").append(StringHelper.ln(1))
        .append(StringHelper.tab(1)).append(TAG_INCLUDE(SQL_ID_SELECT_ORDERBY)).append(StringHelper.ln(1))
        .append(TAG_SELECT_END).append(StringHelper.ln(2))
        */
